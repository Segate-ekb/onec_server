...
find_platform_path() {
    platform_path="/opt/1cv8/x86_64/${ONEC_VERSION}"
    if [ -d "$platform_path" ]; then
        echo "Найдена установленная платформа 1С по пути: $platform_path" >&2
        return 0
    fi

    echo "Платформа 1С не найдена." >&2
    return 1
}
...
change_directory_permissions() {
    echo "Установка необходимых разрешений папкам и файлам..."
    chown -R usr1cv8:grp1cv8 /home/usr1cv8 /opt/1cv8 /var/1C/licenses
    echo "Разрешения успешно установленны."
}
